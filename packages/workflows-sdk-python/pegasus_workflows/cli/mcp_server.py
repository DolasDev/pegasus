...
def resource_reference_integration_config() -> str:
    ...
    return (
        ...
        "Schema: ``GET /api/v1/integrations/mapping-schema`` (each directive carries a "
        "``description``; also ``client.get_mapping_schema()``). Leaf directives: ``$from`` "
        "(path or fallback chain), ``default``, ``$map`` (finite value-translation table), "
        "``coerce``, ``$each``. **Dates**: ``coerce: toDateOnly`` emits ``YYYY-MM-DD`` and "
        "``toIsoDateTime`` emits ``YYYY-MM-DDTHH:mm:ss`` from the native .NET datetime — "
        "wall-clock truncations (a trailing ``Z``/offset is dropped, never applied, so the day "
        "cannot shift) and null-safe (null/absent/empty/unparseable → ``null``). ``coerce`` runs "
        "AFTER ``$map``, so ONE leaf nulls the .NET min-date sentinel and formats every real "
        "date: ``{$from: 'KeyMoveDates.Survey.Planned', $map: {'0001-01-01T00:00:00': null}, "
        "coerce: 'toDateOnly'}``. Nothing in the gate checks your output against the PARTNER's "
        "documented format — the canonical contract types these as generic dates — so the "
        "format is yours to get right.\n"
        ...
        "- ``meta.json`` (optional) — ``{floor, displayName, requiredSecrets?, "
        "requiredConfigs?}``. **Required for a NEW id** with no built-in overlay: name the "
        "floor it builds on. ``requiredSecrets``/``requiredConfigs`` (each a list of "
        ...
        "The gate ENFORCES the source roots, including reads inside ``$each`` — an "
        "element-scope path is composed with the array's own ``$from`` and checked in order "
        "scope. Lean on that: an unresolvable ``$from`` yields ``null``, not an error, so a "
        "typo'd source ships a field quietly missing from the partner body (0042). "
        ...
        "(fork provenance stamped, gate re-run against the current floor). That overlay then "
        "SHADOWS GLOBAL for your tenant.\n\n"
        "    pegasus-workflows integration-config fork <id> [--force] [--yes]\n\n"
        "SDK: ``client.fork_integration_config(id, force=False)``.\n\n"
        "Without ``force`` a fork is ONE-SHOT: if you already own an overlay you get ``409 "
        "CONFLICT``, so an overlay forked from an old GLOBAL can never pick up upstream fixes. "
        "``force=True`` re-seeds it from the CURRENT GLOBAL as a NEW version — prior versions "
        "stay in ``versions`` and remain reachable via ``rollback``, so a bad refresh is "
        "reversible. A stale overlay silently validates against the old contract while GLOBAL "
        "is correct, so re-sync after a platform fix instead of hand-republishing a copy.\n\n"
        "``fork --force`` vs ``delete``: fork keeps your own overlay, rebased on the latest "
        "GLOBAL (history preserved); delete drops the overlay entirely so you re-inherit "
        "GLOBAL live (lineage destroyed). Both re-check the gate, so neither can resurrect a "
        "config the floor has outgrown (``422``).\n\n"
        "## Removing an integration\n\n"
        "``delete`` is the withdrawal path — ONE verb scoped by WHO calls it, removing only "
        "the config lineage your own tenant owns:\n\n"
        "- **platform tenant** → the **GLOBAL** config. Retires a placeholder or renamed id "
        "(e.g. ``demo_partner`` after a rename to ``weichert``) so it stops resolving, stops "
        "appearing in ``list_integrations()``, and can no longer be forked.\n"
        "- **any other tenant** → your own **TENANT** overlay, after which "
        "``get_integration_config(id)`` returns the platform GLOBAL again — the supported way "
        "to drop a stale overlay and re-inherit upstream instead of hand-republishing a copy "
        "that never tracks GLOBAL.\n\n"
        "    pegasus-workflows integration-config delete <id> [--force] [--yes]\n\n"
        "SDK: ``client.delete_integration_config(id, force=False)`` → "
        "``{integrationId, visibility, deleted}``.\n\n"
        "**Irreversible**: it hard-deletes the ENTIRE version lineage, so ``versions`` is empty "
        "afterwards and ``rollback`` cannot undo it. Deleting a GLOBAL that other tenants still "
        "overlay is refused with ``409 DEPENDENTS_EXIST`` unless ``--force``/``force=True`` "
        "(which never touches their rows). An id that also has a built-in definition in "
        "platform code keeps resolving to that code baseline (``published: false``); a "
        "config-only id disappears entirely (get/map/fork all 404).\n\n"
        "## Publishing gate\n\n"
        "Publishing GLOBAL needs the **platform tenant** + a ``vnd_`` key with the "
        "``PublishIntegrationConfig`` action, and the server's "
        "``INTEGRATION_CONFIG_PUBLISH_ENABLED`` switch on — the same gate covers ``delete`` "
        "and ``fork``, though ``fork`` writes a TENANT overlay and so is the one verb the "
        "platform tenant cannot call. ``validate``/``pull`` are read-level.\n\n"
...
def resource_reference_floors() -> str:
    ...
    header = (
        ...
        "a whole native root, a dotted entry opens only that curated sub-path, and the gate "
        "enforces them against EVERY ``$from``, composing reads inside ``$each`` with the "
        "array's own source path. Author "
