...
without platform source, via this README, the MCP `pegasus://reference/*` resources,
...
2. Update its **discovery surfaces in turn** — this README, the MCP resources,
   CLI `--help`, and the OpenAPI spec — preferring **live introspection** (e.g.
   the floor endpoints, and `pegasus://reference/api`, which is generated by
   introspecting `PegasusClient` at serve time) over static docs where the
   contract is code.
