...
describe('ApiStack — reporting switch (reportingEnabled)', () => {
  function synthEnabled() {
    const app = new cdk.App({ context: { 'aws:cdk:bundling-stacks': [] } })
    const apiStack = new ApiStack(app, 'TestApiReportingEnabled', {
      env: { account: '111111111111', region: 'us-east-1' },
  ...
  // Absent — not 'false' — is the off state: the handler tests `=== 'true'`.
  it('leaves REPORTING_ENABLED unset on every Lambda by default', () => {
    const template = synthApiStack()
    const fns = template.findResources('AWS::Lambda::Function')
    const withFlag = Object.values(fns).filter(
      (fn) => fn.Properties?.Environment?.Variables?.REPORTING_ENABLED !== undefined,
    )
    if (withFlag.length !== 0) {
      throw new Error(`expected no Lambda to carry REPORTING_ENABLED, found ${withFlag.length}`)
    }
  })

  it('sets REPORTING_ENABLED=true only on the api Lambda when enabled', () => {
    const template = synthEnabled()
    const fns = template.findResources('AWS::Lambda::Function')
    const enabled = Object.values(fns).filter(
      (fn) => fn.Properties?.Environment?.Variables?.REPORTING_ENABLED === 'true',
    )
    // No cron or executor reads this flag — the api Lambda is the whole surface.
    if (enabled.length !== 1) {
      throw new Error(
        `expected exactly 1 Lambda with REPORTING_ENABLED=true, found ${enabled.length}`,
      )
    }
  })
})
