...
> Safe to do any time. No behavior change. Sets up the local dev pattern.
...
- `packages/web/.env.example` — document that VITE\_\* vars are no longer needed post-Step 4
...
- Add a second source to the existing `BucketDeployment`:
  ...
  ```
- Add a CloudFront cache behavior for the path `/config.json`:
  - `cachePolicy: cloudfront.CachePolicy.CACHING_DISABLED`
  - `originRequestPolicy: cloudfront.OriginRequestPolicy.CORS_S3_ORIGIN`
  - Place this behavior before the catch-all `/*` behavior
...
- Add assertions to the `FrontendStack` test that:
  ...
  - A CloudFront cache behavior exists for path pattern `/config.json` with a disabled cache policy
  - The behavior is ordered before the catch-all SPA behavior
...
- In the `BucketDeployment` sources array, conditionally add `Source.jsonData`:
  ...
  ```
- Add CloudFront cache behavior for `/config.json` (same policy as Step 5)
...
- Add two test cases to the `AdminFrontendStack` test:
  ...
  2. **With Cognito props**: assert `config.json` source is present and the `/config.json`
     CloudFront behavior exists with a disabled cache policy
- Update the snapshot after confirming the assertions pass
