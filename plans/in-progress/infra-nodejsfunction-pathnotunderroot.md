...
1. **Capture the actual resolved values.** Temporarily add, just above line 143 in `api-stack.ts` (or in a throwaway test):
   ```ts
   console.log('cwd', process.cwd(), '__dirname', __dirname, 'entry', path.join(__dirname, '../../../../apps/api/src/lambda.ts'))
   ```
...
- **B. Set the vitest cwd / `root` for `@pegasus/infra`** so synthesis always happens with `process.cwd()` = repo root (e.g. `test: { root: '../..' }` or a `process.chdir` in a setup file). Riskier — changes test harness behaviour globally for the package.
- **C. Pin `aws-cdk-lib` back to the last version that didn't have this behaviour** (only if step 2 shows a regression and A doesn't fully fix it). Least preferred — drags in whatever else changed.
...
- Do not bypass: this is the "fix CI first" rule (CLAUDE.md) — but here CI is *passing* and local is *failing*, so the urgency is "unblock local `git push`", not "stop the line". Still worth doing properly rather than left as a permanent `--no-verify` excuse.
