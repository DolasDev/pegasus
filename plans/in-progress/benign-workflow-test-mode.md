...
- [ ] **Publish to PyPI** (tag `sdk-python-v0.11.0`; watch checks with `--watch` per prior gotcha) — deferred to explicit release go.
...
### Phase B — Outbound delivery primitive `deliver_to_external` (API + Cedar + SDK) — parallel with C
Goal: make partner delivery a platform capability so it is interceptable in dry-run and no longer raw `httpx`. **Blocked on Decision 2.**

- [ ] Cedar: add `DeliverToExternal` action (4-step pattern — `actions.ts`, `cedar.schema.json`, `workflow-runtime.cedar`, route gate). Hot-file coordination on `actions.ts`/`cedar.schema.json`/`*.cedar` — serialize vs any other active stream.
- [ ] API endpoint `POST /api/v1/integrations/:integrationId/deliver-to-external` (mount near `map-to-external`, but under `dualAuthMiddleware` + `requirePermission(Actions.DeliverToExternal)` — **not** the open `apiClientAuthMiddleware` surface). Body `{ external: <body> }`; real run → POST to configured endpoint w/ creds, return `{delivered:true,status,response,dryRun:false}`; unknown integration → 404; missing action → 403. Per Decision 2a add integration outbound-binding schema; per 2b read workflow config/secret server-side.
- [ ] Dry-run awareness: in dry-run the endpoint (or the injected client, per Decision 1) returns `{delivered:false,dryRun:true}` + capture, sends nothing.
- [ ] SDK: `PegasusClient.deliver_to_external(integration_id, body)` in `api.py`; SDK minor + PyPI; MCP `pegasus://reference/api` picks it up via introspection.
- [ ] **In `pegasus-workflows` repo**: rewrite `send_order_to_partner.send_to_partner` to call `deliver_to_external`; drop `httpx` import + `SEND_URL`/`SEND_API_KEY` reads; manifest `required_actions` += `DeliverToExternal`. Verify it delivers the same body.
- Acceptance: unknown id → 404; missing action → 403; real run POSTs to endpoint; dry run sends nothing.
