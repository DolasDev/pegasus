...
- [x] **Set explicit client-side timeouts below the 29 s Lambda ceiling.** MECHANISM DONE — every downstream now fails with a typed, logged error before the 29 s wall instead of riding to it:
  ...
  - **Value choice — deliberately conservative, NOT the aggressive 5–8 s.** The ceilings are *relative to each call's own budget* (never tighter than requested) and sit below the wall, so they cannot break a legitimately-slow query (reports, bulk reads, Neon cold-resume) **without** the spike data the plan requires. **The 5–8 s fast-fail tightening is the remaining follow-up:** once the next >10 s event populates `pegasus/api-slow-requests` with the real legitimate-query ceiling, tighten the per-call `timeoutMs` defaults and **validate in staging first**.
