...
- [ ] Runbook (add to this plan doc or a docs page once written): browser-download the current year's 400NG Baseline Rates workbook → `npx tsx scripts/parse-400ng-xlsx.ts <file> --label "..." --effective-from ... --effective-to ... > 400ng-<yr>.json` → `POST /api/v1/rating/tariffs/import` → review row counts via `GET /rating/tariffs/:id` → `POST /rating/tariffs/:id/activate`.
