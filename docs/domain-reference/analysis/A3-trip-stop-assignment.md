...
> does **not**, and **A3 does not decide that** — it was an A2/A5 question and is now answered by
> [`A2` §3.2](A2-shipment-structure.md)'s rule **B-ONWARD**, which A3 anticipated correctly and in
> the right direction: the new BL is a new **undertaking**, because `src:dtr-part-iv` #81 defines a
> bill of lading as the contract to furnish transportation services. What holds either way is that the **`stay`
...
| The same identity survives a **terminated** stay and reshipment                                                              | **Not claimed here — and now answered NO**        | `src:dtr-part-iv` says a terminated shipment moves **on a new BL** (§E.4(4)(c)) and diversion _excludes_ shipments already in destination SIT. This was A2/A5's question (shared §10.4); [`A2` §3.2](A2-shipment-structure.md) decides it is a **second shipment**, which is the direction this row already leaned. The earlier revision's "survives **every** storage interval" stays withdrawn.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
closed. _Limit, stated and now lifted:_ if the stay was **terminated** rather than ordinary, whether
`store-out` names the same shipment was A2/A5's (§3.2); [`A2` §3.2](A2-shipment-structure.md) answers
it — a **second shipment** — and notes that a consumer cannot tell that branch from this one off the
published records, because the commitment has no record type.
