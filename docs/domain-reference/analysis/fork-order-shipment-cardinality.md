...
written: 2026-09-17
revised: 2026-09-23
revision: 6 — conforms to 00-shared-decisions.md rev 5; A2 closes foreclosure #7
...
> **Revision note.** [`round-2-critique.md`](round-2-critique.md) returned this document
...
> **outranks this file**. Every change §10.2 there requires is applied below — revisions 2 to 5, each
> item citing the §10.2 item it answers, and [`A2` §1](A2-shipment-structure.md) audits the set as
> complete. Every critique point is answered, including the two where revision 2
> **disagrees and says so** (§3.3.1 and §3.2.4). §8 runs the nine scenarios explicitly and §9 lists
> what moved. **Revision 6 is [`A2`](A2-shipment-structure.md)'s**: the one question this document
> left open in its own subject matter — shipment identity across a terminated stay — is now
> answered, by the area the corpus assigns it to.
>
> _(Two counts this note used to carry are deleted rather than corrected: no gate reads them, and an
> ungated count is the defect [`A1` §9](A1-order-service-lifecycle.md) recorded.)_
...
Relocation                     ← NOT an aggregate. A typed external reference on the order.
 └── Order              1 ─── N ── Shipment
      ...
      · award / accept / decline /             evidenced by the transport contract (BOL or
        ...
        00-shared-decisions §1              · kind: NOT PUBLISHED.  [A2 §3.3]
...
> **A second reason, added in revision 6 by [`A2` §3.2(d)](A2-shipment-structure.md), and it is the
> sharper one.** The system B-DOC was read off **separates the commitment from the document itself**.
> DPS offers and the TSP accepts within 24 hours (`src:dtr-part-iv` §C.4); the survey follows; and
> _"the BL cannot be printed until pre-move survey weight and agreed pack/pickup dates are in DPS"_
> (§F.1 NOTE). Award, survey and bill of lading are three events in that order, so even inside DoD
> the document is **downstream evidence of a commitment made earlier**. A rule that reads identity
> off the document is therefore not DoD's rule either; it is a rule that happens to give DoD's
> answers because DoD issues one document per undertaking. A2's **B-ONWARD** keeps the whole of
> B-DOC's content and moves the discriminant to the undertaking — which is what §3.2.3's two-stage
> boundary below was already reaching for, and costs B-DOC nothing it had not conceded.
...
| 7   | ~~**Shipment identity across a terminated SIT stay is NOT decided here.**~~          | A model that never decides it will discover the answer implicitly, in code.                                                                                                                                                                 | **DISCHARGED**, not mitigated. [`A2` §3.2](A2-shipment-structure.md) decides it: a reshipment after termination is a **second shipment**; a diversion, a split at a transshipment point and an ordinary delivery out of storage are not. The cost line beside it — "a model that never decides it will discover the answer implicitly, in code" — no longer applies. What remains is narrower and is A2's own: the rule is decidable in prose and **returns `COMMITMENT_NOT_PUBLISHED`** in code, because §3.2.3's stage-1 minting act has no record type ([`A2` §3.6]).                |
...
5. **Do tenants ever re-issue a transport contract mid-move for administrative reasons?**
   **Narrowed in revision 6.** It was asked because _"under B-DOC that would mint a new shipment"_.
   Under [`A2` §3.2](A2-shipment-structure.md)'s **B-ONWARD** an administrative re-issue over an
   **unchanged undertaking** mints nothing, so this stops being a modelling risk and becomes what it
   should always have been — a question about how often the document and the undertaking come apart
   in a lane we can observe. `src:dtr-part-iv`'s **SF 1200** correction notice and Table A-402-4's
   closed whitelist of correctable BL fields are the corpus's own statement that a correction is not
   a reissue, and A6 owns that regime.
...
7. **Whose commitment mints the boundary when two parties commit differently? — sharpened in
   revision 6.** The RMC awards one service order; the van line registers two registrations against
   it. Stage 1 (§3.2.3) says the boundary is "the set the commitment named", and does not say whose
   commitment wins. [`A2` §3.6](A2-shipment-structure.md) shows the commitment has **no record type
   at all**, so this is not only unanswered but currently unaskable of the data; it should be read
   together with `shipmentCommitment` in [SD §4.7.3] rather than separately. This is an A8 authority question and the user is the source for which
...
| Shipment identity across a **terminated** SIT stay                                 | **not claimed here — decided at [`A2` §3.2]**                 | Was deferred to A2/A5 by `00-shared-decisions.md` §10.4, and revision 1 implied an answer via B-DOC when it should not have. A2 scores its own answer **medium-high**, on a **[SYNTHESIS]** of `src:dtr-part-iv` #81 and §E.4(4)(c), holding it below high because whether a new **award** accompanies the new bill of lading is not in the corpus.                                                                                                               |
...
- **Its type does not come from anywhere**, and this is where that costs something.
  Revisions 1-5 said "its type comes from a closed enum, the published analogues being
  `src:milmove-mymove`'s `BOAT_HAUL_AWAY` / `MOBILE_HOME` with per-subtype tables (ADR 0067)."
  [`A2` §3.3](A2-shipment-structure.md) declines to publish that enum — six sources publish one and
  no two decompose into the same facets, three of the facets being other aggregates' facts — so the
  model expresses that there are two shipments here and **cannot say that one of them is a vehicle
  shipment**. A2 records this scenario as where its largest refusal lands.
...
**Revision 6** — [`A2-shipment-structure.md`](A2-shipment-structure.md). The first revision that
closes a question of this document's own rather than conforming it to something above:

| #   | What moved                                                                                                                                                                                                                                                                                                                                                                                                                                            | Why                                                       |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 28  | **Foreclosure #7 is discharged** and §3.2.4's terminated-SIT row now carries a verdict: a reshipment after termination is a **second shipment**, by rule **B-ONWARD**. §7's "not claimed" row points at A2's own **medium-high** rather than at a deferral. The discriminant is the **undertaking**, and `src:dtr-part-iv` #81 makes the bill of lading the contract that constitutes one — so no rule about documents is needed to get DoD's answer. | [`A2` §3.2]; `[SD §10.4]` bullet 1, now closed both ways. |
| 29  | **§3.2.2 gains a second reason B-DOC is [ORIGINAL]**, and it is the sharper one: DoD itself separates award, survey and document (§C.4, §F.1 NOTE), so the document-reading is not DoD's rule either. B-DOC keeps all of its content and its **medium**; what moves is where the discriminant sits.                                                                                                                                                   | [`A2` §3.2(d)].                                           |
| 30  | **§3.1's "a type from a closed enum (P1)" is withdrawn**, and §8.5 says what that costs on the scenario it was written for: the model expresses two shipments and cannot say that one of them is a vehicle shipment. `shipmentType` is recorded absent and owed.                                                                                                                                                                                      | [`A2` §3.3]; `[SD §4.7.3]`.                               |
| 31  | **§6 q5 narrowed and q7 sharpened.** An administrative re-issue over an unchanged undertaking now mints nothing; and q7 is unaskable of the data until `shipmentCommitment` exists.                                                                                                                                                                                                                                                                   | [`A2` §3.2], [`A2` §3.6].                                 |
| 32  | **Housekeeping.** The frontmatter read `revision: 2` / `revised: 2026-09-18` over a body carrying revisions 3, 4 and 5. The revision note's two ungated counts are **deleted** rather than corrected, per [`A1` §9]'s rule; [`A2` §1] carries the audit that every `[SD §10.2]` item is applied, with the item-to-revision mapping.                                                                                                                   | [`A1` §9]; [`A2` §1].                                     |

**§5.3's open item is closed elsewhere.** It named "the states an order occupies before any shipment
exists" as **[ORIGINAL]** and unresolved, and handed it to A1. [`A1` §3.3](A1-order-service-lifecycle.md)
published five stages and [`A1` §3.4](A1-order-service-lifecycle.md) handed the estimate to A10.
[`A2` §3.4](A2-shipment-structure.md) records that the **zero-shipment order itself needs no reading
beyond §5.3's** — `orderStageAt` never counts shipments — and that what blocks A1's `COMPLETE` fork is
`shipmentCommitment` and A4, not this document's cardinality.
