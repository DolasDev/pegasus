...
| **Rescission of the bill of lading as a cancellation act** — handed here by [`A6` §Cross-area](A6-documents-evidence.md) | **A1**, and it is a new row rather than an owed one | `src:cfr-49-375` §375.505(h) gives the shipper three days after signing to _"rescind the bill of lading without any penalty"_. Under [`A2` §3.2](A2-shipment-structure.md) the bill of lading is the record of the undertaking, so rescinding it withdraws the undertaking — `orderCancellation`'s subject matter. Two things make it A1's rather than A6's and neither is comfortable: the window is keyed to the **document's signature**, for which [`A6` §3.3(b)](A6-documents-evidence.md) found **no record type**, and §375.505(h)'s non-restart clause means a permitted upstream amendment does not reopen it |
