...
**Revision 6** closes **F1**, the first defect found by structural model-checking rather than by
reading (`alloy/`, 2026-09-19). Unlike A–L it was not a conflict between documents: it was two of
_this_ document's own rules being jointly unsatisfiable, and it made an ordinary interstate move
unpublishable.

|       | Was                                                                                                                                                                                                                                                                                                                                                                                                     | Now                                                                                                                                                                                                                                                                                                                                                        |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **M** | §4.7.1's `handover` row declared **no qualifier**, so under §1.3's derived key every handover about one shipment keyed to `(shipment, handover)` and §4.3 selected **one** winner per key. Custody could never change hands twice, §4.8.3's `until` was unreachable, and origin agent → hauler → destination agent could not be published. Relaxing the key naively made the fold multi-valued at a tie | **`handover` declares `{releasing, receiving, side, occurrence?}`**, and the two sides of a transfer are **two facts, not two sides of one**. The fold gains **C5** (the timeline and the transfer gap) and **C6** (the tie), so the multi-valued case is answered rather than avoided — **§4.7.1, §4.7.2f, §4.8.3**, and §1.3 item 3's second worked case |
| **N** | §4.8.3 read `holder` "from the selected handover's **receiving side**" while §4.7.1 put the two `partyRole`s in `context[]`, which §1.4 makes valueless, non-authoritative and **unlabelled** — so nothing could say _which_ ref was the receiver, and the fold read an authoritative output off a non-authoritative field                                                                              | **the parties move into `value`** as `releasingParty` / `receivingParty`, where they are asserted, attributed, contestable and labelled; `partyRole` leaves the `context[]` column, closing the two-homes-for-one-link defect — **§4.7.2f part (4)**, recorded as **F2**                                                                                   |
| **O** | §4.7.1 gave the `handover` row `boundBy = CUSTODY` while §4.8.2 refused that exact binding as circular — "A8-MOVE would be defined in terms of the thing it defines". Harmless while the fold read one handover; load-bearing once it reads a contested pair                                                                                                                                            | **`boundBy` owed, expressly _not_ `CUSTODY`**, with the side-names-the-authority reading carried as **[ORIGINAL]** and barred from scoring; [`A8` §5](A8-authority-skeleton.md) owes a `handover` row — **§4.7.3**, recorded as **F3, open**                                                                                                               |

M and N are decisions; O is a contradiction that stops being hidden behind a filled-in cell, and it
leaves the table with one more owed row than it had, which is the table doing its job. The full
argument, the four candidates and what each of them forecloses are at
[`F1-handover-qualifier-decision.md`](F1-handover-qualifier-decision.md); §4.7.2f carries the
substance, and [`findings-from-alloy.md`](findings-from-alloy.md) carries the finding's own history.
...
The second worked case is **`handover`** (§4.7.2f), and it is the one that shows what the mechanism
is _for_. Successive transfers of one shipment are several independently-contested facts about one
subject, and — unlike a trip delayed twice (§4.7.2d item 3) — the subject **does not change** between
them, so the discriminator cannot be the subject and has to be the qualifier. Its declared qualifier
is `{releasing, receiving, side, occurrence?}`, so the key is `(goods, handover, {…})`, and an
ordinary origin agent → hauler → destination agent move is four keys rather than one contest with
one survivor.
...
| **`handover`** _(custody handoff)_                                                                                                                                                                                                                                                                        | **`goods`**                                                                                                                                                                       | **`{releasing, receiving, side, occurrence?}`** — `releasing`/`receiving` are members of the published role vocabulary ([`A8` §2](A8-authority-skeleton.md)); `side` ∈ `RELEASE` \| `RECEIPT`; `occurrence` is a 1-based ordinal, absent = 1 (**H-OCCUR**). See §4.7.2f | both `stop`s / `trip`s, or the `externallyPerformedLeg` — **and no longer the two `partyRole`s**, which move into `value` (§4.7.2f)                                                                                                                   | **The two sides assert two facts, not two sides of one** (§4.7.2f). Provisionally the role named on the key's `side` is authoritative for that key — **[ORIGINAL]**, do not score. `custodyBasis` 41 vs 349 decides whether authority moves. · `boundBy` **owed, expressly _not_ `CUSTODY`** (circular — §4.8.2) · [A8 §7.1 A8-MOVE, §7.6; A8 §5 owes a row]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
**(f) `handover` declares a qualifier, and the two sides of a transfer are TWO FACTS.** This is the
**F1** decision, argued in full at
[`F1-handover-qualifier-decision.md`](F1-handover-qualifier-decision.md); what follows is its
substance, stated here because §4.7 is where a `qualifier` is declared and §1.3 makes this table the
complete declaration.

**The defect.** Three published rules were jointly unsatisfiable. The fact key is the derived tuple
`(subject, type, qualifier?)` (§1.3); §4.7.1's `handover` row declared **no qualifier**, so every
handover about one shipment keyed to `(shipment, handover)`; and `FactResolved` selects **one**
winner per key (§4.3). So custody could never change hands twice, §4.8.3's `until` ("the next
selected handover's `occurredAt`") was unreachable, and a shipment passing origin agent → hauler →
destination agent — an ordinary interstate move — was unpublishable. Relaxing the key naively made
the fold multi-valued at an `occurredAt` tie with no published tie-break, while [`A8`
§4.3](A8-authority-skeleton.md) hangs eight fact classes' authority on that fold. Found by
structural model-checking, 2026-09-19; recorded at
[`findings-from-alloy.md`](findings-from-alloy.md).
...
2. **The subject does not move.** `handover`'s canonical subject family stays **`goods`** =
   {shipment, portion}.
3. **`handover` declares `{releasing, receiving, side, occurrence?}`**, so the fact key is
...
4. **The two parties move out of `context[]` and into `value`.** `handover`'s `value` is
   `{occurredAt, outcome, reasons[], custodyBasis, releasingParty, receivingParty}`, and §4.8.3's
   fold reads `holder` from `value.receivingParty` of the selected **`RECEIPT`**, never from
   `context[]`.

**Why two facts and not one — four grounds, and the one sentence that reads the other way is the
loser.** §4.8.3's phrase _"the selected handover's receiving side"_ is the sentence that sounds like
one record carrying both ends, and the sentence that beats it is in the same subsection. (A fifth
ground was argued from [`A8` §7.6](A8-authority-skeleton.md)'s worked records and is **withdrawn**
below; the four that remain carry the conclusion without it.)

- **§4.8.3 rule 3 counts them as two, and describes behaviour only two facts can produce.** _"Before
  the first handover, and **across a cross-dock dwell where only one of the two handovers has been
  published**, the fold returns `UNKNOWN`"_ — repeated verbatim at [`A3` §8](A3-trip-stop-assignment.md).
  The documents say _"the two handovers"_, plural; and on the one-fact reading a lone `J1` would be
  a contest of one, `FactResolved` would select it, and the fold would report custody moving on the
  releasing party's word alone. Rule 3 says it does not.
- **~~[`A8` §7.6](A8-authority-skeleton.md) publishes a `FactResolved` for the pair it treats as one
  fact and none for the handover pair.~~ WITHDRAWN, and stated rather than quietly deleted.** As
  argued, this read A8 §7.6's worked records: the two `condition` assertions resolved by a
  `FactResolved`, the two `handover` records carrying none. **It is no longer true of the document it
  cites.** The edit that carried this decision amended [`A8` §7.6](A8-authority-skeleton.md) — its
  revision 6 item 15 — so the two handover records now key differently and **gain a `FactResolved`
  each**; and the same item's two-stop correction has since been carried through §7.6's `condition`
  records, whose `context[]` is `stop:X1` / `stop:X2` and no longer one identical `stop:X`. The
  current text therefore does not say what the evidence said it said. Nor may it simply be restated against the new text: A8 §7.6 was rewritten **by this
  decision**, so citing its amended records back as grounds for the decision is circular. The
  withdrawal costs nothing — the four remaining citations are independent of it, and the strongest of
  them (§4.8.3 rule 3) is settled text this decision did not touch.
- **[`A8` §9 item 9](A8-authority-skeleton.md) answers the question in as many words:**
  `src:stedi-x12-reference`'s open question 5, _"How do we represent custody transfer — one event or
  two?"_ — _"§7.6 answers **two** (the `J1`/`R1` shape), but that is our answer, not stedi's."_
- **The corpus's word is `complementary`, and §4.3's machinery is for `competing`.**
  ...
  assertions by two different [parties]"_ (§4.8.2). §4.3 pairs **competing** assertions and picks a
  winner; complementary assertions have no winner to pick. A release at 14:00 and a receipt at 16:00
  are both true.
- **One fact cannot carry two outcomes, and §2 makes `outcome` mandatory on every act.** A release
  can complete while the receipt fails — the receiving carrier refuses the goods on the dock. Under
  two facts that is `RELEASE COMPLETED` and `RECEIPT NOT_COMPLETED` with a reason, which §2.3 already
  requires. **[ORIGINAL]** as an argument; every mechanism is settled text.

**The counter-evidence, stated rather than buried.** `src:open-trip-model`'s `HandOver` _"indicates
transferring a consignment from one Actor to another"_ and **carries `from`/`to` actor refs** (§8.3)
— one act record with two ends. That is real evidence for the one-fact reading, and it is why
part (4) puts both parties on **both** records' `value`: OTM's shape is preserved field-for-field.
What is not preserved is the claim that one publisher's record settles both sides.
...
Q-KEY is not a new commitment: (i) is §1.1's prohibition on a second subject read at the right
grain, (ii) is what makes the key a _pairing_ device at all (§1.3: _"Competing assertions pair on
this key"_), and (iii) is forced by §4.3, because a key component under contest makes the contest
disappear. The three qualifiers already published — `identity`'s `{scheme, vocabularyScope}`,
`pieceCount`'s `{unitization}`, `charge`'s `{aspect}` — all satisfy it, and none of them contains the
value being asserted. Stating the rule is **[ORIGINAL]**; every input is settled text.

**Three candidates lose, and each loses on Q-KEY.**

- **`{occurredAt}` — rejected, and Q-KEY(iii) is fatal.** `occurredAt` **is** the contested value: it
  sits in `value` on every act record (§4.1, §2) and the whole of §4.2's three-clock apparatus exists
  because parties disagree about it. Keying on it means no two handover assertions ever compete,
  `FactResolved` is always a contest of one, and `supersedes` — constrained to _"the SAME party with
  the SAME fact key"_ (§4.1) — can never link a corrected time to the time it corrects. A handover
  time would be the one act time in the model that can be neither contested nor corrected. Worse on
  the wire we carry: `src:sirva-ade`'s event `DateTime` _"indicates when event was **recorded**"_
  ([`A8` §4.2](A8-authority-skeleton.md)), so the component would frequently be a recording clock
  keyed as an occurrence clock.
- **`{releasing, receiving}` as `partyRole` REFERENCES — rejected, and §4.7.2d(3)'s refusal
  stands.** `partyRole` is an aggregate (§1.2) with a `partyRole`-subject fact class of its own
  (§4.7.3), so a qualifier carrying two `partyRole` refs puts two `SubjectRef`s in the payload, which
  is exactly what §1.1 forbids permanently and what §4.7.2d(3) already refused for `tripDelay`. It
  fails Q-KEY(i); it also fails Q-KEY(ii), because a `partyRole` id is **ours** and the interline
  hauler receiving from a prior carrier does not hold it ([`A8` §9 item
  1](A8-authority-skeleton.md) records that the party entity does not exist yet); and it fails
  Q-KEY(iii), because _which party_ received is itself contestable — that is what
  `value.receivingParty` is for.
- **A custody-transfer sequence number — rejected.** Nothing in the corpus publishes one; the
  receiving agent would have to know it is the third transfer, i.e. know about transfers it was not
  party to (`src:sirva-ade`'s own analysis supplies the operational fact: _"an agent cannot see the
  trip"_, [`A3` §3.3](A3-trip-stop-assignment.md)); and it is circular, because a global ordinal
  counts _selected_ handovers while selection runs per fact key. A platform-minted ordinal is also a
  value nobody asserted published as though somebody had — the shape §4.6.1(a) refuses one field to
  the left.
- **Changing the subject to the juncture — rejected, and it is the closest of the four.** The
  analogy to §4.7.1's `arrival`/`departure` carve-out is real. Three things kill it. (1)
  `externallyPerformedLeg` is a **span**, not a juncture: §8.2 publishes acts against it _"exactly as
  against a stop: **handover-out, handover-in**"_, so one leg carries both sides and the defect
  returns unchanged. (2) One transfer is **two `stopAction`s**: [`A3`
  §3.2](A3-trip-stop-assignment.md) binds a `StopAction` to **exactly one stop** and §8.1 fixes a
  `Stop` at _"one position in one trip's sequence"_, so — as [`A3` §8](A3-trip-stop-assignment.md)
  says in as many words — the releasing side is against a stop on trip 1 and the receiving side
  against a stop on trip 2. There is no single published juncture for both sides to name. (3) The
  receiving interline hauler cannot name our `stopAction`; it cannot see our trip. **And the cost to
  the fold:** §4.8.3 rule 1 restricts the fold's inputs to selected `handover` assertions, and under
  a juncture subject the fold would have to traverse `stopAction → shipment`, so **E-CANON's
  structural admission check would no longer guarantee that a handover is about goods at all**
  (§4.6.2 is purely a check on `subject.aggregate`). You hand over **goods**; you do not hand over a
  `stopAction`.

**Why a qualifier and not two types.** [`A3` §3.2](A3-trip-stop-assignment.md) rules that
`transfer-out` and `transfer-in` are _"**one `handover` type, asserted once by each side** — the
`J1`/`R1` shape … **not two types**"_, and [`A3` §8](A3-trip-stop-assignment.md) repeats it. §4.7.2a's
`pieceCount` is the precedent for the remedy: two independently contested facts about one subject
become **one type with a qualifier**, not two types. So the corpus's two constraints — _one type_ and
_two facts_ — have exactly one solution. **[ORIGINAL]** as the inference; both premises are quoted.

**The components, with their marks.**

| Component    | Domain                                                          | Why it is in the key                                                                                                                                                | Mark                                                                                                        |
| ------------ | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `releasing`  | published role vocabulary ([`A8` §2](A8-authority-skeleton.md)) | Which transfer. `src:open-trip-model`'s `HandOver` carries `from`/`to` actor refs (§8.3); `src:stedi-x12-reference` `R1` names the counterparty's position outright | **[SYNTHESIS]** — the two ends are sourced; rendering them as key components is the join                    |
| `receiving`  | idem                                                            | idem                                                                                                                                                                | **[SYNTHESIS]**                                                                                             |
| `side`       | `RELEASE` \| `RECEIPT`                                          | The two-facts verdict above. Element 1650 publishes the two halves as **two codes**, `J1` and `R1`                                                                  | **Sourced** that the halves are separately reported; **[ORIGINAL]** that they are one type with a qualifier |
| `occurrence` | integer ≥ 1, OPTIONAL, absent = 1                               | Repeatability only (H-OCCUR below)                                                                                                                                  | **[ORIGINAL]**                                                                                              |

**Why role members and not party references.** `assertedBy` is `{partyRef, role}` and _"role rides on
the assertion"_ (§1.1); the role vocabulary is published at [`A8` §2](A8-authority-skeleton.md). A
`roleRef` is a vocabulary member exactly as `scheme` is, so Q-KEY(i) holds, and both parties can
compute it: the releasing agent knows it is handing to the linehaul hauler, and the receiving hauler
knows it took from the prior carrier — which is what the X12 code _names_ (`R1` **Received from Prior
Carrier**), and `src:dp3-tender-of-service` §B.3.f makes the party actually hauling a **nameable**
legal party with a DOT number within 2 GBD, so the counterpart role is a regulatory duty rather than
a convenience.

**Two tests separate this from §4.7.2d(3)'s refused case**, and both are **[ORIGINAL]** as tests while
every input is settled text. (1) _Could the referenced thing be the subject of **this** fact?_ For
`tripDelay` + "the stop it moves": yes — a stop is the canonical subject of `arrival`/`departure`, so
naming it in the qualifier makes the record ambiguous between a trip fact and a stop fact. For
`handover` + role names: no — there is no reading on which "custody of S passed from the origin agent
to the hauler" is a fact **about** a role-holding. (2) _Is it a reference or a vocabulary member?_
§4.7.2d(3)'s refused qualifier named _which_ delay by pointing at another aggregate; this one names a
**position in the act** from a closed list, as `scheme` does.
...
under that scheme"_ (§7.1). A party reference in a `value` is not a second subject; it is an
asserted, contestable fact about the transfer — which is what it must be, because the two sides can
disagree about who took the goods. Note the deletion this makes in §4.7.1's `context[]` column:
`partyRole` leaves it, because two homes for one link is revision 3's defect **C** (§1.1). **Grain is
owed**: §4.8.3 returns `holder` as a `partyRole` while §8.2's `performedBy` is a party, and [`A8`
§9 items 1-3](A8-authority-skeleton.md) owe the party entity and the person-vs-organisation grain.
It is carried as the union, not invented here.
...
> dealings with its own counterparty over this shipment, **never from the catalog's resolved
> history**.

That last clause is the whole difference from the rejected sequence number, which required knowing
about transfers you were not party to. There is no circularity, because the count is over the
asserting party's own acts rather than over `FactResolved`'s output; and because each side is its own
fact, the parties who have to agree on `occurrence` are the corroborators of **one side** — usually
one organisation's driver and its office — not the two sides of an arm's-length interline transfer.
Its residual risk, named rather than hidden: a mis-counted `occurrence` splits one side's assertions
into two keys that never pair. That failure is **detectable** — an `occurrence = n` key with no
`occurrence = n−1` key on the same subject and side is a catalog integrity query — which is the
difference between this component and `{occurredAt}`, whose failure mode is silent. **[ORIGINAL]** as
a query.

**What the requirements check out at.** _Succession:_ origin agent → hauler → destination agent
publishes four facts on four keys — `{originAgent, hauler, RELEASE}`, `{originAgent, hauler,
RECEIPT}`, `{hauler, destinationAgent, RELEASE}`, `{hauler, destinationAgent, RECEIPT}` — two custody
spans, two boundaries. _Pairing:_ the two sides are not required to share a key, but **two parties
speaking about the same side of the same transfer share every component and pair**, contesting in the
ordinary way (§4.3, §4.5). _Independent derivability:_ every component is computable at assertion
time from what the asserting party holds. _Repeatability:_ the same ordered role pair transferring the
same goods twice publishes `occurrence = 1` and `occurrence = 2` on each side — though the common HHG
instances resolve before H-OCCUR is needed, because a shuttle making two trips moves two **Portions**,
which are two subjects under the `goods` family (§3.3).

**What this forecloses**, so a later revision does not reopen it by accident: a handover can never
again be one record agreed by both parties (agreement is two records and a query); custody can never
move on one party's word, so a partner that emits only `J1` or only `R1` leaves permanent `UNKNOWN`
spans — a real operational cost, and the intended one, which **must be stated to ingest before it is
discovered in production**; `handoverOut`/`handoverIn` are foreclosed as type names; the transfer's
time may never become a key component; no catalog-minted custody-transfer sequence number, ever; the
juncture is foreclosed as a subject; `boundBy = CUSTODY` is foreclosed for `handover` itself; and
`occurrence` may not be derived by the platform — it is asserted or it is absent.
...
> **F1 — closed at §4.7.2f.** `handover` declares `{releasing, receiving, side, occurrence?}`; the
> two sides of a transfer are two facts; §4.8.3's fold gains rules **C5** and **C6**. The defect was
> that the fact key is `(subject, type, qualifier?)` (§1.3), §4.7.1's `handover` row declared **no
> qualifier**, and `FactResolved` selects one winner per key (§4.3) — so custody could never change
> hands twice and the fold's `until` was unreachable. Found by structural model-checking on
> 2026-09-19, decided at
> [`F1-handover-qualifier-decision.md`](F1-handover-qualifier-decision.md), recorded as **resolved**
> at [`findings-from-alloy.md`](findings-from-alloy.md).
>
> **Two new defects are opened in its place**, because the same reading turned up both and neither
> should be read as incidental to the repair:
>
> - **F2 — closed by the same edit.** §4.8.3 returned `holder` _"from the selected handover's
>   **receiving side**"_ while §4.7.1 put the releasing and receiving `partyRole`s in `context[]`,
>   which §1.4 rules 2 and 3 make valueless, non-authoritative and **unlabelled** — so nothing could
>   say _which_ of two refs was the receiver, and the fold was reading an authoritative output off a
>   non-authoritative field. §4.7.2f part (4) moves both parties into `value`.
> - **F3 — open.** §4.7.1 gave the `handover` row `boundBy = CUSTODY`, and §4.8.2 says of the
>   identical shape that _"that row's binding would be `CUSTODY`, so **A8-MOVE would be defined in
>   terms of the thing it defines**"_. The two sentences contradict each other, and F1's fix makes
>   the contradiction load-bearing, because the fold now depends on selecting among paired handover
>   assertions. The row's `boundBy` becomes **owed, expressly not `CUSTODY`**, with the
>   side-names-the-authority reading carried as **[ORIGINAL]** and **do not score**; and
>   [`A8` §5](A8-authority-skeleton.md) **owes a `handover` row**, which under **A8-NAMED** every
>   handover `FactResolved` must be able to name.
...
- **Not a fact class.** The act is already in the vocabulary. §4.7's **`handover`** row declares
  ...
  (both sides assert, **and they assert two facts** — §4.7.2f), and custody is computed from the
  result. Note what that costs the `handover` row itself, and it is not free: its own `boundBy`
  cannot be `CUSTODY` either, for the same reason, which is why §4.7.1 now marks it **owed,
  expressly not `CUSTODY`** (F3, §4.7.3).
...
- `src:stedi-x12-reference` element 1650's **`J1`** Delivered to Connecting Line / **`R1`** Received
  ...
  different [parties]"_ — two acts, two asserters, no interval between them on the wire. §4.7.2f
  takes that word `complementary` at face value: they are **two facts**, on two fact keys, and §4.3's
  machinery is for **competing** assertions, which these are not.
...
_(Amended by §4.7.2f. The shape changed in three places and gained two rules: `holder` comes off the
selected **`RECEIPT`**'s `value.receivingParty` rather than off an unlabelled `context[]` entry
(that is **F2** closing); `until` names the next selected **`RELEASE` by that holder** rather than
"the next selected handover", and becomes **reachable** for the first time; and the `UNKNOWN`
outcome carries a reason. Rules 1–4 below stand as written.)_
...
Two rules are added by §4.7.2f. Note what they do to rule 3: its two named `UNKNOWN` cases stop being
stipulations and become **derived**, which is itself evidence for §4.7.2f's two-facts verdict.
...
> `UNKNOWN`**. That gap is rule 3's cross-dock dwell and [`A3` §8](A3-trip-stop-assignment.md)'s
> _"thin part"_, now produced by the fold rather than asserted about it. A `RELEASE` whose holder is
> not the open span's holder does not close it and yields `UNKNOWN`; the fold never reconciles an
> inconsistent sequence.

> **C6 [ORIGINAL] — the tie.** Where two selected handover facts about one goods carry the **same**
> `occurredAt` and would open and close differently, the fold returns `UNKNOWN`. It does not order
> them by key, by `assertedAt`, by `recordedAt` or by `occurrence`. This is **A8-NAMED**
> ([`A8` §4.4](A8-authority-skeleton.md)) in its custody-side form and §4.6.3 step 2's refusal to
> fall through, applied to **ordering** instead of to selection.

The `UNKNOWN` reasons are therefore three: `NO_SELECTED_HANDOVER_AT_OR_BEFORE_INSTANT`,
`IN_TRANSFER_GAP` (C5), `AMBIGUOUS_ORDER_AT_INSTANT` (C6). A fourth,
`RECEIVING_SIDE_NOT_PUBLISHED`, is **retired**: it existed only because the receiving side was read
off `context[]`, and §4.7.2f part (4) closes that (F2).
...
the selected **`RECEIPT`**'s `occurredAt` on that receipt's `custodyBasis` (**A8-MOVE as amended** —
§4.7.2f, and [`A8` §7.1](A8-authority-skeleton.md) is where the amendment is carried). A8's §7 is
unaffected in substance: 41 versus 349 still decides whether authority moves, and §7.6's scenario
still resolves the same way — because the `handover` acts it publishes are exactly the fold's inputs.

**And one thing the fold deliberately does not answer, which authority does.** Across a C5 transfer
gap the fold returns `UNKNOWN`, but **the releasing role remains authoritative** until the receipt
(A8-MOVE as amended), so the gap is a custody `UNKNOWN` and **not** an authority vacuum. The two
questions are two functions over the same published records; fusing them would reintroduce the
vacuum. **[ORIGINAL]**, and marked **do not score** until A8 ratifies it (§4.7 note 3).
...
23. **`Custody` is a projection, not a stored entity** (§4.8). Three places change and the decision
    ...
    `custodyBasis = 349`".
24. **`handover` now declares a qualifier, and two sentences of A3's become consequences rather than
    stipulations** (§4.7.2f). A3 §3.2's `transfer-out` / `transfer-in` row **keeps its ruling** —
    "one `handover` type … not two types" is right and is what the qualifier is built to preserve —
    and must now **cite the qualifier** `{releasing, receiving, side, occurrence?}` as the mechanism
    that makes it work, because "one type asserted once by each side" is unpublishable without it
    (that is F1). And A3 §8's _"where only one of the two handovers has been published the fold
    returns `UNKNOWN`"_ must be restated as a consequence of **C5** (§4.8.3) rather than as a rule
    A3 states: the gap between a `RELEASE` and the next `RECEIPT` is now **produced** by the fold.
    A3's "thin part" and its agent-to-agent handoff row are unchanged in outcome.
...
Four more, from §4.7.2f (the **F1** decision). The first is a gap A8 must fill; the second is an
amendment to one of A8's own rules and is marked **[ORIGINAL]**, **do not score**, until A8 ratifies
it (§4.7 note 3); the third and fourth are corrections to §7.6.

9. **[`A8` §5](A8-authority-skeleton.md) owes a `handover` row.** It has none today — §4.7.1's
   handover row cites §7.1 and §7.6 precisely because §5 has no row — and under **A8-NAMED** every
   handover `FactResolved` must name a rule. The row cannot be `boundBy = CUSTODY`: that is the
   circularity §4.8.2 refuses, and F1's fix makes it load-bearing, because the fold now depends on
   selecting among paired handover assertions. §4.7.1 carries the provisional reading — the role
   named on the key's own `side` is authoritative for that key, which breaks the circle because it
   reads the **key**, not the fold, and is `src:stedi-x12-reference`'s own arrangement (only the
   releasing carrier issues `J1`; only the receiving carrier issues `R1`). Whether that is a sixth
   `boundBy` member or `NONE` plus a named rule is A8's to decide.
10. **A8-MOVE is amended to name the receipt instant.** §7.1 says authority moves "effective at the
    handoff instant"; there are now **two** instants, the release and the receipt, and the rule must
    name one. It reads: _authority over every `boundBy = CUSTODY` fact class moves at the selected
    **`RECEIPT`**'s `occurredAt`, on that receipt's `custodyBasis`: `349` moves it to the receiving
    role; `41` moves custody and not authority. Across a transfer gap (**C5**) the **releasing role
    remains authoritative** until the receipt, so the gap is a custody `UNKNOWN` and not an authority
    vacuum._ Nothing new is sourced — 41 vs 349 is `src:uncefact-rec24`, already quoted at §7.1. The
    reasons for choosing the receipt are internal and are the two that decided the two-facts
    verdict: moving authority at the release would grant it to a party that has not spoken (**M1**),
    and §7.4(b) already holds that a former holder "never stops being authoritative for facts before
    the handoff".
11. **§7.6's two handover records take the qualifier, gain a `FactResolved` each, and its `stop:X`
    on two trips is corrected.** Each record carries `qualifier = {originAgent, hauler,
RELEASE|RECEIPT, —}`; they key differently, so each is resolved on its own key rather than left
    as two records nobody is asked to choose between. And `context = [stop:X, trip:T1]` /
    `[stop:X, trip:T2]` writes **one stop on two trips**, which §8.1 forbids ("a visit to one place,
    at one position in one trip's sequence"); it is two stops, exactly as [`A3`
    §8](A3-trip-stop-assignment.md) already says ("the releasing side against a stop on trip 1, the
    receiving side against a stop on trip 2"). A transcription slip in the worked example, not a
    licence.
12. **[`A8` §9 item 9](A8-authority-skeleton.md)'s _"§7.6 answers two"_ is upheld, and now has a
    mechanism.** It should cite §4.7.2f: the answer is still ours rather than stedi's, but it is no
    longer an assertion about a worked example — it is the fact key.
...
| §4.7.2f `handover`'s qualifier, and the two-facts verdict                                                                                                                                    | **Medium-high — [ORIGINAL] in the join.** The underlying facts are sourced: `src:stedi-x12-reference` element 1650 publishes `J1`/`R1` as two codes by two parties and its analysis calls them "two complementary assertions"; `src:open-trip-model`'s `HandOver` carries `from`/`to` actor refs; [`A3` §3.2](A3-trip-stop-assignment.md) rules "one `handover` type … not two types"; §4.7.2a is the precedent for one-type-plus-qualifier. **[ORIGINAL]:** Q-KEY, `side`, H-OCCUR, C5 and C6. The decision is also **forced** rather than chosen — "one type" plus "two facts" has exactly one solution — and it is checked structurally in `alloy/custody.als`. **What would overturn it:** a published HHG model in which one party's record settles both ends of a transfer, or a consumer that proves it cannot tolerate the `UNKNOWN` spans foreclosure 2 creates — in which case the fix is a downstream named rule, never the fold selecting on one side's word.                                                                              |
| §4.7.2f's A8-MOVE amendment (the receipt instant) and `handover`'s own authority                                                                                                             | **Not scored — [ORIGINAL] and provisional.** §4.7 note 3 and [`A8` §10](A8-authority-skeleton.md)'s last row both bar it. Naming the receipt instant and the side-names-the-authority reading are ours; A8 ratifies or replaces them, and [`A8` §5](A8-authority-skeleton.md) owes the row (F3).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
