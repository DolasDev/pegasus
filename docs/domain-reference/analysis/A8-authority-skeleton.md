...
**(iv)** There is **no `handover` row, and one is owed** ([shared §4.7.2f](00-shared-decisions.md),
recorded as **F3**). It may not be `boundBy = CUSTODY`: that is the circularity
[shared §4.8.2](00-shared-decisions.md) refuses, and it became load-bearing when `handover` took a
qualifier, because the fold now selects among a contested pair. [Shared
§4.7.1](00-shared-decisions.md) carries the provisional reading — the role named on the key's own
`side` is authoritative for that key, which is `src:stedi-x12-reference`'s own arrangement (only the
releasing carrier issues `J1`; only the receiving carrier issues `R1`) and breaks the circle because
it reads the **key**, not the fold. **[ORIGINAL]**, **do not score**; whether it is a sixth `boundBy`
member or `NONE` plus a named rule is this document's to decide. Until it lands, **A8-NAMED** (§4.4)
has no rule for a handover `FactResolved` to name.
