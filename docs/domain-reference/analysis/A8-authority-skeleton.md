...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `CUSTODY`    | Authority follows **`custodyAt(goods, instant)`** ([shared §4.8.3](00-shared-decisions.md)) — the named, versioned fold over the `FactResolved`-selected `handover` assertions and over `ExternallyPerformedLeg.custodyBasis` — evaluated at the instant the fact is _about_ (A8-INSTANT) and moving at a boundary per A8-MOVE on the selected handover's `custodyBasis`. There is no stored `Custody` entity. | arrival, departure, load/unload performance, delivery performance, condition, SIT entry inputs, SIT release | §7; [shared §4.8](00-shared-decisions.md)                                                                 |
...
> **Rule A8-MOVE. At a custody boundary with `basis = 349` (handed over to another party), authority
> over every `boundBy = CUSTODY` fact class moves to the receiving role, effective at the handoff
> instant. At a boundary with `basis = 41` (continued responsibility), custody moves and **authority
> does not**.**
...
```
Act   type=handover        subject=shipment:S   context=[stop:X, trip:T1]   basis=ACTUAL
      outcome=COMPLETED
...
Act   type=handover        subject=shipment:S   context=[stop:X, trip:T2]   basis=ACTUAL
      outcome=COMPLETED
...
Assertion type=condition    subject=item:i7   context=[shipment:S, stop:X]   basis=ACTUAL
      assertedBy={originAgent A, role: OriginAgent}   ← releasing
Assertion type=condition    subject=item:i7   context=[shipment:S, stop:X]   basis=ACTUAL
      assertedBy={hauler H, role: Hauler}             ← receiving
...
**Two shapes this scenario used to publish and no longer may.** Both were the old two-axis envelope
showing through, and both are now unpublishable rather than merely untidy.
...
the goods across the dwell, its inputs are the two **`handover`** acts above, and the authority
function is defined over the fold regardless of what the dwell is eventually called. _(Where only one
of the two handovers has been published, the fold returns `UNKNOWN` and does not fall through to the
last known holder — shared §4.8.3 rule 3, which is A8-NAMED's prohibition on implicit
last-writer-wins in its custody-side form.)_
...
7. **Authority over money beyond §5 row 11.** Revenue allocation is a different question from charge
   ...
   read as settling it.
8. **Fact classes §5 does not cover.** The ten the blocker named are here. Not here: cube, piece
   count, packing performance, survey/estimate facts, ETA, seal integrity
   (`src:dp3-tender-of-service` #39-40 has a full actor/deadline model for it), tracer results,
   claim facts, and every A10/A11 class. Each needs a row before its area can score a dependent
   decision high.
9. **`src:stedi-x12-reference` element 98 is still unread**, and it remains the one list that would
   ...
   (the `J1`/`R1` shape), but that is our answer, not stedi's.
