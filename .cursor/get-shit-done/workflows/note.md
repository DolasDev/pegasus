...
- **Global scope**: `/home/steve/repos/pegasus/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/steve/repos/pegasus/.cursor/notes/`)
...
2. Glob `/home/steve/repos/pegasus/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (/home/steve/repos/pegasus/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/steve/repos/pegasus/.cursor/notes/` — works in any directory
