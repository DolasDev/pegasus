...
function parseActivities(activities: any[] = []) {
  ...
  return {
    ...
    orderIdToColor: [...orderIds].reduce((accum: any, orderId: any, i: number) => ({ ...accum, [orderId]: getColor(i) }), {}),
    hasDateChange: hasDateChange,
  };
}

function TripInternal(props: any) {
  ...
  useEffect(() => {
    async function fetchTrip() {
      ...
      const trip = await API.fetchTrip(Number(props.match.params.tripId));
      setTrip(trip);
      } catch (e: any) {
        setShowError(e)
      }
