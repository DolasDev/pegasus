...
export function ActivityGantt({ days, activities, orderIdToColor, reloadTrip }) {
  ...
  const updateActivity = async(partialActivity) => {
  ...
  const syncActivityDates = async() => {
    ...
    await dispatch(updateActivityForTrip(selectedActivity.activityId, partialActivity));
    reloadTrip();
  }
  ...
  const etaDate = selectedActivity?.estimated_date ? new Date(selectedActivity.estimated_date) : null;
  ...
  return (
    ...
    <div className={styles.activityGantt}>
      <div className={styles.header}>
        {days.map((day, i) => (
          <ActivityHeader key={day} date={day === null ? 'Unknown' : formatDateShort(day)} />
        ))}
      </div>
      {activities.map((activity) => {
        const isOrderSelected = selectedActivity && selectedActivity.order_num === activity.order_num;
        const isSelectedActivity = selectedActivity && selectedActivity.activityId === activity.activityId;
        return (
                        ...
                        <div >
                          ...
                          (
                            ...
                            <Button
                                ...
                                onClick={
                                  () => {
                                    updateActivity({is_confirmed: selectedActivity.is_confirmed ? false : true})
                                  }
                                }
                              >
                                <HoverToolTip content='Confirmed with Driver' direction="right">
                                <i className={selectedActivity.is_confirmed ? 'fas fa-flag-checkered' : 'fa fa-question' }></i>
                                </HoverToolTip>
                              </Button>
                            </span>
                          ) : ''  } 
            ...
            {selectedActivity.is_confirmed || !selectedActivity.activityType?.isHasETA ? (
              ...
              <HoverToolTip content='Confirm Actualized Date' direction="right">
                <DatePicker
                  ...
                  openToDate={actualDate ? actualDate : new Date(selectedActivity.planned_start)}
                />
                </HoverToolTip>
              </div>
            </div>  
...
const ActivityRow = React.forwardRef(({ activity, onClick, className, days }, ref) => {
  const width = 80;
  const padding = 10;
  const numberOfDays = getTotalDays(activity);
  const offset = getOffset(activity.planned_start, days);
  const etaDate = activity.actual_date ? new Date(activity.actual_date) : activity.estimated_date ? new Date(activity.estimated_date) : null;
  const etaOffset = getOffset(etaDate, days) - offset;
  const isEtaSet = !!etaDate;
  const hasDateChange = activity.hasDateChange
  const newNumberOfDays = hasDateChange ? (datediff(activity.newStart, activity.newEnd) || 0) + 1 : 0;
  const newOffset = hasDateChange ?  getOffset(activity.newStart, days) : 0;

  const innerContent = (
    <>
      <div>
        <span>{activity.activityType?.abbreviation}  {`${getFormattedWeight(activity?.shipment?.pegasus_shadow?.weight || activity?.shipment?.total_est_wt)} `}</span>
        <HoverToolTip content={activity.actual_date ? `Verified Complete` : activity.is_confirmed ? `Confirmed With Driver` : activity.is_committed ? `Driver Commitment Made`: ''} direction="right"> 
          <i className={activity.actual_date ? `fas fa-truck-moving ${styles.green}` : activity.is_confirmed ? `fas fa-flag-checkered ${styles.green}` : activity.is_committed ? `fa fa-check ${styles.green}`: ''}></i>  
        </HoverToolTip> 
      </div>
      <div>
  ...
  return (
    <>
      ...
      <div
        ...
        <div ref={ref}><div><span>{`New Dates!`}</span></div></div>
      </div>
      : null}
    </>
  );
