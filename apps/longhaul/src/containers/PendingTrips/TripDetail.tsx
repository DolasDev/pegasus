import React, { useState, useEffect, useRef } from 'react';
import { useSelector } from 'react-redux';
import styles from './TripDetail.module.css';
import { useOutsideClick } from "../../utils/hooks/use-outside-click";
import { Select } from '../../components/Select';
import type { RootState } from '../../redux/store';

interface TripDetailProps {
  editable?: boolean;
  currentTrip: any;
  label: string;
  property?: string;
  editLabel?: string;
  EditComponent?: React.ComponentType<any>;
  editTrip: any;
  displayVal?: React.ReactNode;
  editVal?: any;
  placeholder?: string;
}

export function TripDetail({
  editable = true,
  currentTrip,
  label,
  property,
  editLabel,
  EditComponent,
  editTrip,
  displayVal,
}: TripDetailProps) {
  const [editMode, setEditMode] = useState(false);
  const [value, setValue] = useState<any>();
  const save = () => {
    setEditMode(false);
    editTrip({ [property as string]: value });
  };
  const edit = () => {
    setEditMode(true);
  };

  const onChange = () => {
    setValue(value);
    editTrip({ [property as string]: value });
  };

  useEffect(() => {
    setValue(currentTrip[property as string]);
  }, [currentTrip, property]);

  return (
    <div className={styles['trip-detail']}>
      {editMode && EditComponent ? (
        <>
          <div className={styles['title']}>{label}</div>
          <EditComponent onChange={onChange} value={value} /> <a onClick={save}>Save</a>
        </>
      ) : (
        <>
          <div className={styles['title']}>{label}</div>
          {displayVal} {editable && <a onClick={edit}>{editLabel}</a>}
        </>
      )}
    </div>
  );
}

export function DriverTripDetail({
  editable = true,
  currentTrip,
  label,
  property,
  editLabel,
  EditComponent,
  editTrip,
  displayVal,
}: TripDetailProps) {
  const [editMode, setEditMode] = useState(true);
  const [value, setValue] = useState<any>(currentTrip.driver);
  const save = () => {
    setEditMode(false);
    editTrip({ [property as string]: value });
  };
  const edit = () => {
    setEditMode(true);
  };
  const onChange = (value: any) => {
    setValue(value);
    //setEditMode(false);
    editTrip({ [property as string]: value, driver_id : value?.id || null});
  };

  useEffect(() => {
    setValue(currentTrip[property as string]);
  }, [currentTrip[property as string]]);

  return (
    <div className={styles['trip-detail']}>
      {editMode && EditComponent ? (
        <>
          <div className={styles['title']}>{label}</div>
          <EditComponent onChange={onChange} value={{ value: value, label: value?.driver_name || '' }} />
        </>
      ) : (
        <>
          <div className={styles['title']}>{label}</div>
          {displayVal} {editable && <a onClick={edit}>{editLabel}</a>}
        </>
      )}
    </div>
  );
}

export function NameTripDetail({
  editable = true,
  currentTrip,
  label,
  property,
  editLabel,
  EditComponent,
  editTrip,
  displayVal,
  editVal,
}: TripDetailProps) {
  const [editMode, setEditMode] = useState(true);
  const [value, setValue] = useState<any>(editVal);
  const [actualEditMode, setActualEditMode] = useState(false);

  const save = () => {
    setEditMode(true);
    editTrip({ [property as string]: value });
  };
  const edit = () => {
    setEditMode(true);
  };

  const onChange = (value: any) => {
    setValue(value);
    setActualEditMode(true)
  };

  const wrapperRef = useRef<HTMLDivElement>(null);

  useOutsideClick([wrapperRef], () => {
    if(actualEditMode){
      editTrip({ [property as string]: value })
      setActualEditMode(!actualEditMode)
    }
  });

  useEffect(() => {
    setValue(currentTrip[property as string] || 'Pending Trip');
  }, [currentTrip, property]);


  return (
    <div ref={wrapperRef} className={styles['trip-detail']}>
      {editMode && EditComponent ? (
        <>
          <div className={styles['title']}>{label}</div>
          <EditComponent onChange={onChange} value={value} />
        </>
      ) : (
        <>
          <div className={styles['title']}>{label}</div>
          {displayVal} {editable && <a onClick={edit}>{editLabel}</a>}
        </>
      )}
    </div>
  );
}

export function DispatcherTripDetail({
  editable = true,
  currentTrip,
  label,
  property,
  editLabel,
  EditComponent,
  editTrip,
  displayVal,
}: TripDetailProps) {
  const [editMode, setEditMode] = useState(true);
  const [value, setValue] = useState<any>();
  const onChange = (value: any) => {
    setDispatcher(value?.value);
    editTrip({ [property as string]: value?.value, dispatcher_id: value?.value.code || null});
  };

  useEffect(() => {
    setDispatcher(currentTrip[property as string]);
  }, [currentTrip[property as string]]);

  const common_state = useSelector((state: RootState) => state.common);
  const dispatcherListAsOptions = (common_state.dispatcherList || [] ).map((dispatcher: any) => ({ label: `${dispatcher.first_name} ${dispatcher.last_name}`, value: dispatcher }));
  const [dispatcher, setDispatcher] = useState<any>(currentTrip.dispatcher || null)

  return (
    <div className={styles['trip-detail']}>
      <div className={styles['title']}>{label}</div>
        <Select
            placeholder="Planner"
            defaultValue={null}
            options={dispatcherListAsOptions}
            value={ dispatcher?.code ? {value: dispatcher ,  label: `${dispatcher.first_name || ''} ${dispatcher.last_name || ''} `} : {value:null, label:''}}
            onChange={onChange}
          />
    </div>
  );
}
