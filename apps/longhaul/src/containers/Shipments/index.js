...
const MemoizedShipmentCards = React.memo(({ shipments }) => {
  const shipmentToTrips = useSelector(
    state => state.tripPlanning.shipmentToTrips
  );
...
export const SearchDashboard = () => {
  ...
  const release_channel = useSelector(state => state.version.release_channel);
  ...
  useEffect(() => {
    if (debouncedQuery) {
      dispatch(fetchShipments(debouncedQuery))
    }
