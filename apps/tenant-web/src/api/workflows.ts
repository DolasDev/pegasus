...
export interface WorkflowManifest {
  ...
  /**
   * Mermaid flowchart of the workflow, generated by the SDK from the source and
   * embedded in the bundle. Required on workflows published after the workflow
   * visualization feature; absent on earlier rows (which render the verified
   * envelope only).
   */
  diagram?: string
}
...
/**
 * One flattened Temporal history event for the execution-inspection timeline —
 * mirrors `WorkflowHistoryEvent` in apps/api/src/lib/temporal-client.ts.
 */
export interface WorkflowHistoryEvent {
  id: string
  /** Event kind, e.g. `ActivityTaskFailed`. */
  type: string
  /** ISO-8601 event time, or null when the proto carried none. */
...
/**
 * Fetch the Temporal event-history timeline for an execution. Empty when the
 * run never started on Temporal. Requires `workflow:read`.
 */
export async function listExecutionHistory(
  id: string,
  executionId: string,
): Promise<WorkflowHistoryEvent[]> {
  const { events } = await apiFetch<{ events: WorkflowHistoryEvent[] }>(
    `/api/v1/workflows/${id}/executions/${executionId}/history`,
  )
  return events
}

/**
 * Request cancellation of a non-terminal execution (202). Cancellation is
 * cooperative — the row stays RUNNING until the reconcile poller flips it to
 * CANCELLED. Requires `workflow:cancel_execution`.
 */
export async function cancelExecution(id: string, executionId: string): Promise<WorkflowExecution> {
  return apiFetch<WorkflowExecution>(`/api/v1/workflows/${id}/executions/${executionId}/cancel`, {
    method: 'POST',
  })
}
...
export async function retryExecution(id: string, executionId: string): Promise<WorkflowExecution> {
  return apiFetch<WorkflowExecution>(`/api/v1/workflows/${id}/executions/${executionId}/retry`, {
    method: 'POST',
  })
}
