...
function renderFilterComponentByType(type: any, args: any, common_state: any) {
  const stateListAsOptions = (common_state.stateList || []).map((state: any) => ({
    label: `${state.geo_name} ${state.geo_code}`,
    value: state.geo_code,
  }))
  const TripStatusAsOptions = (common_state.tripStatuses || []).map((status: any) => ({
    label: status.status,
  ...
  function dateOnChange(e: any, index: any) {
    ...
    args.onChange([
      ...val.slice(0, index),
      daysBetween(date, e.target.value),
      ...val.slice(index + 1),
    ])
  }
...
export function FilterTabs() {
  ...
  return (
    <div data-target="filter-tabs">
      ...
      {showFiltersModal && (
        <FilterModal modalIsOpen={showFiltersModal} closeModal={() => setShowFiltersModal(false)} />
      )}

      {
        ...
        showSaveFilterModal && (
          <SaveFilterModal
            modalIsOpen={showSaveFilterModal}
            closeModal={() => setShowSaveFilterModal(false)}
          />
        )
      }
    </div>
