...
import { addShipmentToTrip as addShipmentToTripAction } from '../../../../redux/trip-planning'
...
function getNameShort(value: any): string {
  ...
  const shipperShort = value
    ? getTitleCaseWord(shipperNameParts[0]) +
      ', ' +
      (shipperNameParts.length > 1 ? shipperNameParts[1][0] : '')
    : ''
  return shipperShort
}
...
export function ShipmentCard({
  ...
  const columns: any[] = [
    <span>
      <b>{`${shipment.shipper_state}`}</b>, <>{`${getTitleCaseWord(shipment.shipper_city)}`}</>{' '}
    </span>,
    <span>
    ...
    shipment.latest_activity_abbr
      ...
      : `${formatDate(shipment.latest_activity_date, { defaultVal: '' })}`,
    ...
    <Card
      ...
      ${
        shipment.TripStatus_id === 1
          ? ''
          : shipment.TripStatus_id === 2
            ? styles.offered //yellow
            : shipment.TripStatus_id === 3
              ? styles.accepted //orange
              : shipment.TripStatus_id === 4
                ? styles.inprogress //green
                : shipment.TripStatus_id === 5
                  ? styles.disabled
                  : ''
      }`}
