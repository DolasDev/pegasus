...
import { createRoom, findRoomById, listRoomsByMoveId, addItem } from '../inventory.repository'
...
describe.skipIf(!hasDb)('InventoryRepository (integration)', () => {
  ...
  it('addItem returns null for an unknown room id', async () => {
    const result = await addItem(testDb, '00000000-0000-0000-0000-000000000000', {
      name: 'Ghost item',
    })
    expect(result).toBeNull()
  })
